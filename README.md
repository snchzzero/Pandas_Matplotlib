...
<details>
 ...
 django, celery, redis, docker, postgresql, psycopg2, pandas, matplotlib, seaborn, httplib2, discovery, ServiceAccountCredentials, csv, time, datetime, random, html, bootstrap
</details>
...
* [Инструкция по запуску скрипта/веб сервиса (через docker)](#Инструкция_по_запуску_скрипта)
* [Графики рассеяния](#Графики_рассеяния)
* [**Web сервис на Django**](#Web_сервис_на_Django)
* [**Web сервис на Django - скриншоты**](#Веб_интерфейс)
...
<a name="Инструкция_по_запуску_скрипта">**Инструкция по запуску скрипта/веб сервиса**</a>
...
```
**Для запуска веб сервиса на Django (запуск через docker):**
1. перейти в папку \Pandas_Matplotlib и в терминале ввести следующие команды:
```shell
...
Для решения используются библиотеки: **django, celery, redis, psycopg2**

Весь функционал заданий реализован через веб сервис на Django, используется очередь задач Celery в связке с брокером сообщений Redis.

В web интерфейсе доступно по нажатию кнопок:
* "импорт и анализ данных" - подключение к документу GoogleSheets с исходными данными, парсинг данных, запись в 'csv', анализ данных в соответствии с [заданием №1](#Задание_№1);
* "показать веб таблицу" - отображение таблицы с отсортированными данными по умолчанию в соответствии с требованиями [задания №1](#Задание_№1), так же доступна пользовательская сортировка в окне браузера по 4ем параметрам, где к каждому параметру можно применить дополнительную сортировку по "возрастанию" или "убыванию". В качестве одного из четырех параметров можно выбрать следующие колонки: 'area', 'cluster', 'cluster_name', 'keyword', 'count', 'x', 'y', 'color'. Сортировка применяется с помощью библиотеки pandas, после сортировки данные заносятся в БД PostgreSQL;
* "отправить данные в GoggleSheets" - в фоновом режиме отправляет данные в новый документ GoogleSheet, в правом верхнем углу появиться ссылка для перехода в документ для просмотра перенесенной таблицы;
* "создать графики рассеяния" - в фоновом режиме создает графики в соответствии с требованиями [задания №2](#Задание_№2)
* "показать графики" - показывает созданные графики

[Инструкция по запуску веб сервиса](#Инструкция_по_запуску_скрипта)

[оглавление](#оглавление)

<a name="Веб_интерфейс">Web сервис на Django - Скриншоты</a>

![Screenshot](pic1.jpeg)
