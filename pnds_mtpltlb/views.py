...
def show_web_tab(request):
    ...
    if request.method == 'GET':
        ...
        form = Form_Action(request.POST)
        form2 = Form_Sort(request.POST or None)
        if form.is_valid():
            analysis_f = form.cleaned_data.get("Analysis_Model")
            show_web_tab_f = form.cleaned_data.get("ShowWebTab_Model")
            send_data_gs_f = form.cleaned_data.get("SendDateGS_Model")
        ...
        print(sorts_1_f, sorts_AD_1_f, sorts_2_f, sorts_AD_2_f, sorts_3_f, sorts_AD_3_f, sorts_4_f, sorts_AD_4_f)
        if analysis_f == "analysis":
            analyst_data()
            db_insert_data()
            name = "импорт входных данных Google Sheets, анализ данных - завершен"
            return render(request, 'pnds_mtpltlb/web_tab.html', {'form': Form_Action(), 'name': sorts_1_f,
                                                                 'now': time(), 'rows': rows,
        ...
        # если применена сортировка и осталось сортировка по умолчанию
            ...
            sorts_4_f != 'count' or sorts_AD_4_f != "убыванию") or \
            ...
            try:
                ...
                return render(request, 'pnds_mtpltlb/web_tab.html', {'form': Form_Action(), 'form2': Form_Sort(),
                                                                  'now': time(), 'rows': rows,
                                                                  'name': 'Error while working with PostgreSQL'})
            finally:
                if connection:  # закрываем подключение к БД
                    connection.close()
